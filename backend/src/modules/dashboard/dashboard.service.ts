...
export class DashboardService {
    ...
    async getDailyStats() {
        ...
        return {
            ...
            pedidos: {
                ...counts,
                enRuta: Math.max(0, counts.totales - (counts.entregados + counts.cancelados + counts.fallidos))
            },
    ...
    async getActiveOperations() {
        ...
        return rutas.map(ruta => {
            ...
            const entregados = ruta.detalles_ruta.filter(d => d.pedidos?.estado_pedido === 'entregado').length;
            const fallidos = ruta.detalles_ruta.filter(d => ['fallido', 'cancelado'].includes(d.pedidos?.estado_pedido)).length;
